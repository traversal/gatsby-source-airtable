...
exports.sourceNodes = async (
  ...
  { apiKey, tables, concurrency, rawValues }
) => {
      ...
      return all.reduce((accumulator, currentValue) => {
        return accumulator.concat(
          currentValue[0].map(row => {
            ...
            row.tableName = currentValue[5]; // tableName from tableOptions above
            row.typeName = currentValue[6]; // typeName from tableOptions above
            ...
            row.rawValues = rawValues !== false && currentValue[7] !== false; 
